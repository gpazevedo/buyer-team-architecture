...
**Correction 2026-08-19 (impl PR #333) — the update above was true for about a day, and the root cause was not the one AD-24 inferred.** `var.alert_email` defaulted to `""` and was documented as something to set in a local `terraform.tfvars`, but `infra/.gitignore` ignores `*.tfvars`, so no CI run ever saw a value. Both subscriptions are `count = var.alert_email != "" ? 1 : 0`, which means every workflow apply did not merely skip creating them — it planned the existing subscription **away**, destroying whatever had been applied by hand. That is why the hand-applied subscriber of 2026-08-05 was gone by 2026-08-06; [AD-24](../02-agent-architecture-behavioral-control/AD-024-steering-hook-failure-semantics.md)'s update that day attributed the disappearance to dev's VPC teardown, which was a coincident but incorrect diagnosis. The consequence spans every alarm in this repo, not just this ADR's evaluation alarms: from 2026-08-01 (the earliest date the repo records the zero-subscriber state) to 2026-08-19, including the `alerts_critical` compliance tier added for `governance_confidentiality_violation`, alarms fired into the console and paged nobody. This ADR's premise that a 0-subscriber topic distinguished AD-34's evaluation alarms from the rest was therefore describing the norm rather than an anomaly — the deletions still stand on cost grounds (an alarm nobody reads is a billing line either way), but not on that comparative one.

The fix is a real committed default. A GitHub Actions variable was rejected for reproducing the same failure shape — it degrades to `""` when unset, so the destroy-on-apply behaviour would return silently. The `count` guard stays, so an environment can still opt out explicitly. One residual manual step, which Terraform cannot close: an SNS email subscription is created in `PendingConfirmation` and AWS mails a link a human must click, once per topic.
