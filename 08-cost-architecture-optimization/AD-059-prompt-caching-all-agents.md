...
Enable Bedrock prompt caching for all 6 agents, caching the invocation-invariant request prefix (system prompt + tool schemas) at the maximum TTL supported by the configured model: 5 minutes for Nova, the only deployed model family (AD-58's "Scenario A/B" framing for this choice was never realized — see AD-58's correction note). A cache write carries a premium over standard input (~1.25× at the 5-minute TTL), so an agent that never lands a second read before its prefix expires can cost marginally more than uncached. The prefix invariant — every per-invocation input falls after the cache checkpoint — is owned and tested by `DynamicAgentFactory` (AD-65, PRD-010 §3.4 / REQ-C004; agent-side: AD-28, PRD-003 §3.4 / REQ-A707).
...
| Gained | Given up |
| --- | --- |
| ~90% off input tokens on a cache read — the primary cost lever; transparent to application code above the factory | Cache writes carry a ~1.25× premium at the 5-minute TTL; an agent whose prefix expires before any second read costs marginally more than uncached |
