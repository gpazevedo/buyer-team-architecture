...
>
> **Update (2026-07-10, PR #195):** the default moved again, and the single-file framing above is now stale too. The literal default now lives in `evals/judge_config.py::DEFAULT_EVAL_LLM_MODEL_ID`, a shared constant imported by both `evals/llm_judge.py` and `evals/tone_negotiation_judge.py` (previously each hardcoded its own copy) — set to `us.anthropic.claude-haiku-4-5-20251001-v1:0` (Claude Haiku 4.5), not Sonnet 4.5. Haiku is still Claude-family, so the cross-family-from-Nova-agents independence property this ADR cares about is unaffected; the change is a cost optimization (~3x cheaper per the module's own docstring), not an independence-mechanism change. `EVAL_LLM_MODEL_ID` env var override behavior is unchanged.
