...
**Correction 2026-08-14 (impl PR #293): "Retry exhaustion routes to DLQ → REQUIRES_ATTENTION" was only half true.** `resilience/redrive.py::_route_to_requires_attention` — the function every retry-exhaustion path in `redrive_dlq` calls — only ever published to the `requires-attention-dlq` SQS queue; it never wrote `NEGOTIATIONS_TABLE`. That inverts this ADR's own stated invariant (echoed in AD-16 and the platform's public write-up): the DynamoDB `entry_trigger` write is supposed to be authoritative and SQS fire-and-forget, not the reverse. A negotiation whose retries were exhausted reached the DLQ queue but never actually entered `REQUIRES_ATTENTION` as a status — nothing showed up on an ops dashboard filtering by status. PR #293 adds the missing `update_item` (`entry_trigger="retry_exhaustion"` for `max_redrives_exceeded`; `no_checkpoint`/`unparseable_message`/`missing_negotiation_id` route through the same call with distinct `entry_reason` text) so the DynamoDB write now happens alongside the SQS publish, making the sentence above true for the first time. See AD-16's 2026-08-14 wiring-closure note for the sibling triggers fixed in the same PR.
