...
**Third call site, and a security case this tiering did not anticipate (2026-08-19, impl PR #333).** PO delivery through the receiving Gateway degrades to an in-process boto3 write on any gateway exception, which is correct under this ADR — a Gateway or Cognito outage must not break the PR→PO handoff. It was observable only as a `log.warning`. `orchestrator/po_delivery.py` now classifies the exception into `auth` (401/403), `http`, `config` and `transport`, and emits `procurement/resilience po_delivery.gateway_fallback` dimensioned by `reason` — the same shape as `strategy.fallback_engaged` above, and for the same purpose: a Cedar DENY must not read like a network timeout.

Two notes. First, a drift worth naming: this ADR's Decision says degradation events carry `degradation_type`/`fallback_used`/`tenant_id`, but both classified call sites converged on `reason` instead. The `reason` shape is the one to follow — it is what two independent implementations reached for — and the Decision text is what is stale.

Second, and the more important point: this is the first call site where availability-first degradation can defeat a *security* control rather than merely lower quality. The fallback bypasses the Gateway's Cedar engine, its REQUEST interceptor and its JWT authorizer in one step, so under `ENFORCE` a scope/tag misconfiguration would route every PO around the whole authorization chain while the configuration still read as "enforcing." This ADR's trade-off table anticipates quality degrading silently; it does not anticipate a control being silently cancelled. That case is recorded separately as [AD-149](../05-security-governance-trust-boundaries/AD-149-alarm-the-bypass-before-enforce.md), which makes the `auth`-bucket alarm a precondition for any enforcement flip. Making the fallback fail closed instead was rejected there for this ADR's own reason — it would convert a dependency outage into a halted handoff.
