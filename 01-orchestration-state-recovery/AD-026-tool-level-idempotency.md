...
**Correction 2026-08-04 (impl PR #257): the third Trade-offs row — "Operators can resume from checkpoint and reuse partial work after a DLQ escalation (REQ-A950b/c)" — was not achievable until this PR.** AD-14's checkpoint write was dead code, so `redrive_dlq` found no checkpoint for any message and escalated all of them to REQUIRES_ATTENTION with `no_checkpoint`; there was no partial work to reuse because none was ever recorded. This ADR's own mechanism was unaffected and worked as described — tool-level dedup keys were live throughout, which is exactly why replaying from the top instead of resuming was merely wasteful rather than unsafe. With checkpoints now written by all six node Lambdas, the row states a real capability; it has not yet been exercised against a live DLQ redrive.
