...
**Update 2026-08-20 — the delivered PO was missing two of its canonical fields (impl PR #340, the 2026-08-20 integration review).** `delivery_address` and `issued_at` are named canonical by PRD-011 §3.2/§4 but `orchestrator.node_award_comms._build_received_po` never set either, so every PO handed to the receiving domain arrived with no ship-to address and no issue timestamp — a gap present since this ADR's own PR #92/#95. Fixed by threading `delivery_address` from `award_comms` through `_build_received_po`/`_deliver_to_receiving` into the canonical export dict, and stamping `issued_at` at assembly time (delivery is idempotent on `order_id`, so a replay of an already-received PO never overwrites the first `issued_at`). Both fields are optional on `test_tenant_skill.po_receiving.IncomingPurchaseOrder`, so POs delivered before this fix continue to validate unchanged rather than requiring a backfill. Separately, `orchestrator/po_delivery.py`'s module docstring said the Gateway-fronted receive path was "off by default, live cutover is a later flag-flip" — stale since PR #67 actually flipped `deliver_via_gateway = true` for every deployed environment; corrected to state the true default and that the in-process boto3 path is now only the fallback.
