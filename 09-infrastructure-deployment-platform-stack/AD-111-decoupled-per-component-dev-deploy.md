...
**Update 2026-08-16 (impl PR #325): the apply step rides out the NAT-Gateway/EIP teardown race.** The untargeted `terraform apply -auto-approve plan.tfplan` step now retries up to 5× with a 25s pause between attempts — the transient-AWS-side-latency wait `release_vpc.sh`/`restore_vpc.sh` already proved for this exact race (`ReleaseAddress` failing with `InvalidNetworkInterfaceID.NotFound` while AWS is still tearing down the NAT's ENI; retrying instantly re-hits the not-yet-torn-down ENI). The apply can carry a destroy/reconcile pass when the NAT-pause state (`enable_nat_gateway`/`enable_interface_endpoints`, read from the warmer rule's state by the `vpcstate` step) changes under the deploy, so a first-attempt failure here is expected AWS-side latency, not a state conflict — the same self-healing shape as the "Commit map back" push retry this ADR's Results already records.
