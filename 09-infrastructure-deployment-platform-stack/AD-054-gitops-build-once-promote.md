...
**Gap closed 2026-07-12 (buyer-team-impl working tree, uncommitted):** this ADR's "in-place image mutation rejected" decision was accepted from the start, but prod's ECR repos inherited dev's per-repo `immutable = false` default until now — a re-pushed tag could have silently violated this decision in production, undermining the traceability guarantee. Prod now forces `IMMUTABLE` on every repository regardless of the per-repo default (`infra/ecr.tf`), closing that gap. As a direct consequence, `prod-deploy.yml`'s image-promotion step was made retry-safe (per-tag digest check: skip an already-promoted matching digest, hard-fail on a mismatch) so a legitimate retry after a downstream failure doesn't hit `ImageTagAlreadyExistsException`.
