...
**Update 2026-08-20 — the canonical Gateway ingest path silently dropped both fields it was supposed to carry (impl PR #340, the 2026-08-20 integration review).** `lambdas/pr_event_router` forwards `correlation_id` as a tool argument on the canonical (`INGEST_VIA_GATEWAY`, on in every deployed environment via `infra/variables.tf`) path, but `skill_runtime`'s `ingest_purchase_requisitions` MCP tool had no matching parameter — FastMCP's `ArgModelBase` inherits pydantic's `extra="ignore"`, so the value was discarded without error. Only the in-process fallback (which calls `_start_workflow` directly, bypassing the tool's arg model) carried it. Same class of bug one hop later: the skill's own `_start_workflow` never injected `_otel_ctx` into the SFN payload the way the router's fallback does, so a Gateway-path ingestion started a fresh root span at Node 1 even after the field-name fix. Both fixed: `ingest_purchase_requisitions` gained a `correlation_id: str | None = None` parameter, and `_start_workflow` now mirrors the router's `propagate.inject(carrier)` call. The router's `skill_runtime/server.py` docstrings for both functions were updated to state explicitly that the two implementations must be kept at parity by hand — this bug is the second instance of that exact failure mode (the first being AD-146's blocking-call gap), and a third, unreachable copy of `ingest_purchase_requisitions` still exists at `skills/integration/ingest_pr.py` with a different signature, importable by nothing live. Generation of `correlation_id` for real-tenant PRs (as opposed to propagating one once it exists) remains the one gap still open from the paragraph above.
