...
**Theme:** Observability & Evaluation  **Catalog:** AD-115 · **Source PRD:** PRD-004 · **Status:** Accepted — fallback mechanism superseded, see corrections below · **Related:** AD-29, AD-34, AD-120, AD-121
...
>
> **Second correction (2026-07-09, PR #181):** the correction above called the missing-alarm gap moot, reasoning that EMF's stdout `print` left no distinct CloudWatch-native signal to fail independently of ordinary process errors. That reasoning ruled out reviving *this ADR's specific mechanism* — it did not rule out a differently-shaped signal. **AD-121** adds one: a schedule-triggered heartbeat Lambda, independent of business traffic, watched by a `treat_missing_data = "breaching"` alarm. The gap is closed, but by a dead-man's-switch on pipeline liveness, not a revival of the fallback-datapoint mechanism this ADR originally described.
