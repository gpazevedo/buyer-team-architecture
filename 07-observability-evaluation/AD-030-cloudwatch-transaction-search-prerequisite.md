...
100%-sampled Transaction Search turned out to be dev's single biggest CloudWatch cost driver (ApplicationSignals custom metrics + `/aws/spans` + `/aws/application-signals/data` ingestion, all billed even while the environment sits idle). PR #235 (2026-07-21) has `release_vpc.sh` deliberately withdraw this prerequisite when the dev VPC is paused — flipping the destination to `XRay` and the default indexing rule to 0% — and `restore_vpc.sh` puts it back (`CloudWatchLogs` / 100%) on restore, with both directions guarded to skip when already in the target state. This is a scoped, intentional exception rather than a reversal: nothing runs against a paused dev VPC anyway, so the "evaluations silently produce no scores" failure mode this ADR warns about cannot occur during the pause window — the prerequisite is back in force before anything that depends on it (AD-32, AD-34) can execute again.
