...
**Update 2026-08-19 (impl PR #332): this pipeline is no longer the only way out of the deadlock it was scoped to break, and that is deliberate.** The staging gate now treats SUSPENDED calibration as advisory while the panel declares `synthetic_scores: true` (AD-32, AD-90), so promotions are unblocked today on judge consistency alone. That removes the schedule pressure but none of the value: judges remain uncalibrated, which is exactly the condition this ADR's Context describes. The coupling is now mechanical rather than aspirational — the reconcile step overwrites the panel and drops the placeholder flag, and calibration blocking re-arms automatically the moment it does. Running the labeling job is what converts an advisory warning back into a gate.

**Update 2026-08-29 (impl PR #402, E1-8) — the 2026-08-19 update above was still framed as "running the labeling job" being deferred, not declined; that framing is now wrong.** User decision, re-confirmed against live AWS before asking (`sagemaker:ListLabelingJobs` still returns 0 jobs): this pipeline **will not be run for the foreseeable future**. REQ-O221, its Transcript Panel Dataset dual-human-scoring requirement, and its contribution to REQ-O223's re-baselining triggers are marked retired in PRD-004 (v1.5.9, PRDs repo). The advisory-calibration downgrade this ADR's 2026-08-19 update describes is now the **permanent** intended behavior, not a stopgap pending real labels — AD-32's Results carry the matching correction. `infra/ground_truth_labeling/` and its three scripts are left in place, unremoved, deliberately — if this decision is ever reversed, the pipeline this ADR describes is still the mechanism, not a rebuild. REQ-O220 (judge self-consistency, no human reference needed) is entirely unaffected and continues to gate as before.
