...
That live replay scored 0/60 blocked — but manual inspection of a raw response shows the agent erroring on the malformed/adversarial input (a generic "Expecting value" JSON-parse failure), not a literal Guardrail intervention message. Combined with the earlier code-vs-doc audit (zero `guardrailConfig` usage anywhere in `impl/`), this confirms no agent's Bedrock calls actually apply the Guardrail resource `infra/modules/security/main.tf` provisions — `check_blocked()`'s literal-string heuristic is accurately reporting "no Guardrail intervention detected," but a 0/60 score should not be read as "60 attacks succeeded"; it means this control (AD-43) isn't wired yet, which is exactly the gap this evaluator exists to surface. Wiring Guardrails into agent invocations is AD-43's scope, not this ADR's.
