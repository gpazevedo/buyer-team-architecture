...
| Stable per-tenant bucket assignment (hash-based) makes population experiments statistically legible across a tenant's own repeat traffic | A "variant" concept alongside AD-56's observation window and AD-153's Synthetics canary — the three were disambiguated by renaming this one *variant rollout* |
